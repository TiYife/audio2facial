...
from util.config import get_flame_config
...
class FLAME(nn.Module):
    ...
    def forward(
        ...
        if expression_params is None:
            expression_params = torch.zeros(batch_size, self.cfg.expression_params).to(shape_params.device)

        if pose_params is None:
            pose_params = torch.zeros(batch_size, self.cfg.pose_params).to(shape_params.device)
        ...
        transl = transl if transl is not None else self.transl.expand(batch_size, -1)
        full_pose = torch.cat(
...
def get_default_flame():
    config = get_flame_config()
    flame = FLAME(config)
    return flame
