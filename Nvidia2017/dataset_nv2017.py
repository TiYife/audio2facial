...
class DataSplitRecorder:
    ...
    def load(cls, path: str):
        ...
        recorder.id_map = recorder.init_id_map()
        recorder.data_map = recorder.init_data_map()
    ...
    def init_id_map(self):
        data_list = self.get_list("all")
        id_map = {}
        idx = 0
        for human_id, sentence_id, audio_index, data_verts_index in data_list:
            key = human_id + sentence_id
    ...
    def init_data_map(self):
        datamap = {}
        for human_id, sentence_id, audio_index, data_verts_index in self.get_list("all"):
            key = human_id + sentence_id
            if key not in datamap:
                datamap[key] = {}
            if audio_index not in datamap[key]:
                datamap[key][audio_index] = data_verts_index
        return datamap
...
class ClipVocaSet(Dataset):
    def __init__(
        ...
        if self.split_frame:
            self.datalist = self.remove_invalid_data()
        else:
            unique_datalist = set()
    ...
    def get_single_item(self, key) -> VocaItem:
        ...
        return VocaItem(
            ...
            sentence_id=self.split_recorder.id_map[human_id + sentence_id],
            frame_id=audio_index,
        )
    ...
    def __getitem__(self, idx):
        if self.split_frame:
            key1 = self.datalist[idx]
            human_id, sentence_id, audio_index, data_verts_index = key1
            key2 = (human_id, sentence_id, audio_index+1, self.split_recorder.data_map[human_id+sentence_id][audio_index+1])
            return self.get_single_item(key1), self.get_single_item(key2)
        else:
    ...
    def get_predict_data(self, human_id: str, wav_path: str):
        res = []
        if self.split_frame:

    def remove_invalid_data(self):
        frame_map = {}
        for human_id, sentence_id, audio_index, data_verts_index in self.datalist_raw:
            key = human_id + sentence_id
            if key not in frame_map:
                frame_map[key] = audio_index
            else:
                frame_map[key] = max(frame_map[key], audio_index)

        res_datalist = []
        for human_id, sentence_id, audio_index, data_verts_index in self.datalist_raw:
            key = human_id + sentence_id
            if key in frame_map and frame_map[key] == audio_index:
                continue
            res_datalist.append((human_id, sentence_id, audio_index, data_verts_index))
...
class VocaDataModule(L.LightningDataModule):
    ...
    def get_max_frame_num(self):
        data_list = self.train_dataset.split_recorder.get_list("all")
        frame_num = 0
        for human_id, sentence_id, audio_index, data_verts_index in data_list:
            frame_num = max(frame_num, audio_index)
        return frame_num

    def get_max_sentence_num(self):
        return len(self.train_dataset.split_recorder.id_map)
